...
VALUES(N'Reservation', @ReservationId, N'玩家已确认', N'玩家确认收到预约回复', NULL, @UserId, GETDATE());";
        ...
        /// <summary>
        /// 玩家在小程序端确认本次到店体验已完成，便于随后提交评价。
        /// </summary>
        public bool CompleteReservationByPlayer(int reservationId, int userId, out string message)
        {
            const string sql = @"
UPDATE dbo.Reservations
SET Status = N'已完成',
    ConfirmStatus = N'已完成',
    CheckedInAt = ISNULL(CheckedInAt, GETDATE()),
    PlayerConfirmedAt = ISNULL(PlayerConfirmedAt, GETDATE())
WHERE Id = @ReservationId
  AND UserId = @UserId
  AND Status NOT IN (N'已取消', N'已完成');

IF @@ROWCOUNT = 0
BEGIN
    RAISERROR(N'未找到可完成的预约订单。', 16, 1);
    RETURN;
END

INSERT INTO dbo.BusinessActionLogs(BusinessType, BusinessId, ActionType, ActionTitle, ActionContent, OperatorUserId, CreatedAt)
VALUES(N'Reservation', @ReservationId, N'玩家到店完成', N'玩家在小程序确认到店完成', NULL, @UserId, GETDATE());";

            return ExecuteNonQuery(sql, command =>
            {
                command.Parameters.AddWithValue("@ReservationId", reservationId);
                command.Parameters.AddWithValue("@UserId", userId);
            }, "到店完成已确认，可以提交评价。", out message);
        }

        /// <summary>
        /// 后台运营确认订单已完成。完成后订单不再计入场次占用名额。
        /// </summary>
        public bool CompleteReservationByAdmin(int reservationId, int adminUserId, out string message)
        {
            const string sql = @"
DECLARE @CurrentStatus NVARCHAR(30);
...
BEGIN
    RAISERROR(N'该预约订单已进入终态，不能重复完成。', 16, 1);
    RETURN;
END
...
SET Status = N'已完成',
    ...
    RepliedAt = GETDATE(),
    ProcessedByUserId = @AdminUserId,
    AdminRemark = N'后台确认订单完成并释放名额。',
    AdminReply = N'本次剧本体验已完成，名额已释放，欢迎提交评价。'
WHERE Id = @ReservationId;

INSERT INTO dbo.AdminReplyLogs(BusinessType, BusinessId, AdminUserId, ReplyContent, VisibleToUser, CreatedAt)
VALUES(N'Reservation', @ReservationId, @AdminUserId, N'本次剧本体验已完成，名额已释放，欢迎提交评价。', 1, GETDATE());

INSERT INTO dbo.BusinessActionLogs(BusinessType, BusinessId, ActionType, ActionTitle, ActionContent, OperatorUserId, CreatedAt)
VALUES(N'Reservation', @ReservationId, N'已完成', N'后台完成预约订单', N'后台在剧本占用情况中完成订单并释放名额。', @AdminUserId, GETDATE());";

            return ExecuteNonQuery(sql, command =>
            {
                command.Parameters.AddWithValue("@ReservationId", reservationId);
                command.Parameters.AddWithValue("@AdminUserId", adminUserId);
            }, "预约订单已完成，原占用名额已释放。", out message);
        }
...
DELETE FROM dbo.Scripts WHERE Id = @ScriptId;";
        ...
        private static ReservationInfo MapUserReservationInfo(SqlDataReader reader)
        {
            ...
            item.HasReview = GetBoolean(reader, "HasReview");
            return item;
        }
