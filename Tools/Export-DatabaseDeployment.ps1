# 从当前 SQL Server 数据库导出部署脚本。
# 该脚本会读取表结构和数据并生成可部署 SQL，运行前请确认连接字符串指向正确环境。
...
param(
    ...
    [string]$OutputPath = 'D:\毕业设计\dramamurder\DramaMurderGraduation.Web\Database\Generated\Deploy-DramaMurderGraduationDb.sql'
)
...
function Get-TypeDefinition {
    ...
    switch ($typeName) {
        'nvarchar' { return '{0}({1})' -f $typeName, ($(if ($Column.max_length -eq -1) { 'MAX' } else { [int]$Column.max_length / 2 })) }
        'nchar' { return '{0}({1})' -f $typeName, ([int]$Column.max_length / 2) }
        'varchar' { return '{0}({1})' -f $typeName, ($(if ($Column.max_length -eq -1) { 'MAX' } else { [int]$Column.max_length })) }
        'char' { return '{0}({1})' -f $typeName, ([int]$Column.max_length }
        'varbinary' { return '{0}({1})' -f $typeName, ($(if ($Column.max_length -eq -1) { 'MAX' } else { [int]$Column.max_length })) }
        'binary' { return '{0}({1})' -f $typeName, ([int]$Column.max_length) }
        'decimal' { return '{0}({1},{2})' -f $typeName, $Column.precision, $Column.scale }
        'numeric' { return '{0}({1},{2})' -f $typeName, $Column.precision, $Column.scale }
        'datetime2' { return '{0}({1})' -f $typeName, $Column.scale }
        'datetimeoffset' { return '{0}({1})' -f $typeName, $Column.scale }
        'time' { return '{0}({1})' -f $typeName, $Column.scale }
...
function Get-IndexColumnList {
    ...
    $selected = $Rows | Where-Object { [bool]$_.is_included_column -eq $Included } | Sort-Object @{ Expression = { if ($Included) { [int]$_.index_column_id } else { [int]$_.key_ordinal } } }
    if (-not $selected) {
        return ''
    }
...
$lines.Add('GO')
$lines.Add('')
$lines.Add('-- Generated from current LocalDB schema by Tools/Export-DatabaseDeployment.ps1')
$lines.Add('')
...
"@ @{ ObjectId = $objectId }
    ...
    foreach ($group in ($keyRows | Group-Object ConstraintName)) {
        $first = $group.Group[0]
        $constraintName = [string]$first.ConstraintName
        $constraintType = if ([string]$first.ConstraintType -eq 'PK') { 'PRIMARY KEY' } else { 'UNIQUE' }
        $indexType = if ([string]$first.IndexTypeDesc -match 'CLUSTERED') { 'CLUSTERED' } else { 'NONCLUSTERED' }
...
foreach ($group in ($foreignKeyRows | Group-Object ForeignKeyName)) {
    $first = $group.Group[0]
...
foreach ($group in ($indexRows | Group-Object SchemaName, TableName, IndexName)) {
    $first = $group.Group[0]
